...
def recalc(filename, timeout=30):
    ...
    # 处理 Linux 和 macOS 之间 timeout 命令的差异
    ...
    if result.returncode != 0 and result.returncode != 124:  # 124 是 timeout 退出码
        error_msg = result.stderr or '重新计算时发生未知错误'
        ...
        for err_type, locations in error_details.items():
            if locations:
                result['error_summary'][err_type] = {
                    'count': len(locations),
                    'locations': locations[:20]  # 最多显示 20 个位置
                }
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  - error_summary: 按错误类型分类及其位置")
